...
class NAApiClient
{
    ...
    public function setCurlOpt($params = array())
    {
        $this->extraCurlOpt = $params;
    }
    ...
    public function makeRequest($path, $method = 'GET', $params = array())
    {
        ...
        if (!empty($this->extraCurlOpt)) {
            curl_setopt_array($ch, $this->extraCurlOpt);
        }
        $result = curl_exec($ch);
        $errno = curl_errno($ch);
        ...
        if (preg_match('#HTTP/1.\d 2\d\d#', $headers[0])) {
            ...
            if (!$decode) {
                if (preg_match('/^HTTP\/1.1 ([0-9]{3,3}) (.*)$/', $headers[0], $matches)) {
                    ...
                    throw new NAJsonErrorType(200, 'OK');
                }
            }
    ...
    public function getAccessToken()
    {
        ...
        if ($this->access_token) {
            if (!is_null($this->expires_at) && $this->expires_at < time()) {
        ...
        } elseif ($this->getVariable('username') && $this->getVariable('password')) {
            //grant_type == password

            return $this->getAccessTokenFromPassword($this->getVariable('username'), $this->getVariable('password'));
        } else {
            throw new NAInternalErrorType('No access token stored');
        }
    }
